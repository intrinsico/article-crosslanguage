...
%There is also a large amount of information that could be used for dictionary construction besides of article titles, such as the content inside each article. Considering that similar content is usually available in abundance for parallel corpora, it is possible to retrieve cross-language information using a
%bilingual topic model \cite{vulic}. Regarding the inter-language links in Wikipedia, it has been demonstrated that they are also useful resources for bilingual dictionary construction (\cite{erdmann}).
